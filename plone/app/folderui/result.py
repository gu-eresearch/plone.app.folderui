...
if __name__ == '__main__':
    from Lazy import Lazy, LazyCat, LazyMap
    ...
    cat = FakeCatalog()
    cat[11235813] = FakeBrain()
    cat[12345678] = FakeBrain()
    cat[11223344] = FakeBrain()
    
    result = cat()
    orig_rids = list(result._seq)
    r = IterableCatalogResults(result, cat)
    ## iterate through keys, values, items
    assert list(r.iterkeys()) == r.keys() == orig_rids
    assert list(r.itervalues()) == list(r.values())
    
    result = cat()
    orig_rids = list(result._seq)
    r = IterableCatalogResults(result, cat)
    assert list(r.iteritems()) == zip(orig_rids, list(r.values()))
